...
def parse_binary_command(in_buffer, is_response=True):
    ...
    """
    #print("protocol.parse_binary_command: %r, %r" % (in_buffer, is_response))
    ...
    cmd_args = dict((param_label, param_value.decode()) for param_label, param_value in zip(expected_cmd_params, split_arguments))
...
def pack_binary_command(cmd_type, cmd_args, is_response=False):
    ...
    *NOTE* Expects that all arguments in cmd_args are already str's.
    """
    #print("protocol.pack_binary_command: %r, %r, %r" % (cmd_type, cmd_args, is_response))
    expected_cmd_params = GEARMAN_PARAMS_FOR_COMMAND.get(cmd_type, None)
    if expected_cmd_params is None or cmd_type == GEARMAN_COMMAND_TEXT_COMMAND:
    ...
    if compat.any(not isinstance(param_value, str) for param_value in cmd_args.values()):
        raise ProtocolError('Received non-binary arguments: %r' % cmd_args)

    data_items = [cmd_args[param].encode() for param in expected_cmd_params]
...
def parse_text_command(in_buffer):
    ...
    #print("protocol.parse_text_command: %r" % in_buffer)
    cmd_type = None
    cmd_args = None
    cmd_len = 0
    if b'\n' not in in_buffer:
...
def pack_text_command(cmd_type, cmd_args):
    """Parse a text command and return a single line at a time"""
    #print("protocol.pack_text_command: %r, %r" % (cmd_type, cmd_args))
