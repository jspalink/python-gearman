...
class CommandHandlerStateMachineTest(_GearmanAbstractTest):
    ...
    def recv_server_response(self, _response_line):
        if isinstance(_response_line, str):
            response_line = _response_line.encode()
        else:
            response_line = _response_line
        print("recv_server_response: %r" % response_line)
