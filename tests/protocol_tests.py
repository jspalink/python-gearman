...
class ProtocolBinaryCommandsTest(unittest.TestCase):
    ...
    def test_parsing_errors(self):
        ...
        imaginary_command_buffer = struct.pack('!4sII4s', protocol.MAGIC_RES_STRING, protocol.GEARMAN_COMMAND_TEXT_COMMAND, 4, 'ABCD'.encode())
        ...
        unexpected_payload_command_buffer = struct.pack('!4sII4s', protocol.MAGIC_RES_STRING, protocol.GEARMAN_COMMAND_NOOP, 4, 'ABCD'.encode())
    ...
    def test_parsing_single_arg(self):
        echoed_string = 'abcd'
        echo_command_buffer = struct.pack('!4sII4s', protocol.MAGIC_RES_STRING, protocol.GEARMAN_COMMAND_ECHO_RES, 4, echoed_string.encode())
    ...
    def test_parsing_single_arg_with_extra_data(self):
        echoed_string = 'abcd'
        excess_bytes = 5
        excess_data = echoed_string.encode() + (protocol.NULL_CHAR * excess_bytes)
    ...
    def test_parsing_multiple_args(self):
        ...
        self.assertEquals(cmd_args, dict(job_handle='test', task='function', unique='identifier', data=expected_data.decode()))
    ...
    def test_packing_errors(self):
        ...
        cmd_type = protocol.GEARMAN_COMMAND_JOB_CREATED
        cmd_args = dict(job_handle=bytes(12345))
        self.assertRaises(ProtocolError, protocol.pack_binary_command, cmd_type, cmd_args)
    ...
    def test_packing_single_arg(self):
        ...
        expected_command_buffer = struct.pack(expected_format, protocol.MAGIC_REQ_STRING, cmd_type, expected_payload_size, cmd_args['data'].encode())
        packed_command_buffer = protocol.pack_binary_command(cmd_type, cmd_args)
        self.assertEquals(packed_command_buffer, expected_command_buffer)
    ...
    def test_packing_multiple_args(self):
        ...
        expected_payload = protocol.NULL_CHAR.decode().join(ordered_parameters).encode()
...
class ProtocolTextCommandsTest(unittest.TestCase):
    ...
    def test_packing_single_line(self):
        ...
        self.assertEquals(packed_command, expected_string.encode())
